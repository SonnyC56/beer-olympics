...
async function checkTournamentAdmin(tournamentId, userId) {
...
export const adminRouter = router({
    ...
    getTournamentDashboard: protectedProcedure
        ...
        try {
            ...
            const tournament = await getDocument(`tournament::${input.tournamentId}`);
            // Get comprehensive stats
            const [teamsResult, matchesResult, pendingScoresResult, disputesResult] = await Promise.all([
                query(`SELECT COUNT(*) as count FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
             WHERE _type = 'team' AND tournamentId = $1`, { parameters: [input.tournamentId] }),
             ...
             WHERE _type = 'match' AND tournamentId = $1`, { parameters: [input.tournamentId] }),
                query(`SELECT COUNT(*) as count FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default s
             JOIN \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default m ON m.id = s.matchId
             WHERE s._type = 'score_submission' AND s.status = 'PENDING' AND m.tournamentId = $1`, { parameters: [input.tournamentId] }),
                query(`SELECT COUNT(*) as count FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default d
             WHERE d._type = 'dispute' AND d.status = 'OPEN' AND d.tournamentId = $1`, { parameters: [input.tournamentId] })
            ]);
            // Get activity timeline
            const recentActivity = await query(`SELECT * FROM (
          ...
          LIMIT 20`, { parameters: [input.tournamentId] });
            ...
            if (tournament.isMegaTournament && tournament.subTournamentIds) {
                const subStats = await Promise.all(tournament.subTournamentIds.map(async (subId) => {
                    const subTournament = await getDocument(`tournament::${subId}`);
                    const subMatchesResult = await query(`SELECT COUNT(*) as total, COUNT(CASE WHEN isComplete = true THEN 1 END) as completed
                 FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
                 WHERE _type = 'match' AND tournamentId = $1`, { parameters: [subId] });
                    return {
                        id: subId,
    ...
    updateTournamentSettings: protectedProcedure
        ...
        try {
            ...
            if (input.updates.name)
                tournament.name = input.updates.name;
            if (input.updates.date)
                tournament.date = input.updates.date;
            if (input.updates.isOpen !== undefined)
                tournament.isOpen = input.updates.isOpen;
    ...
    getTournamentAnalytics: protectedProcedure
        ...
        try {
            ...
            const teamPerformance = await query(`SELECT 
           ...
           ORDER BY wins DESC, totalPoints DESC`, { parameters: [input.tournamentId] });
            // Get match completion trends
            const matchTrends = await query(`SELECT 
           ...
           ORDER BY date`, { parameters: [input.tournamentId] });
            // Get player participation stats
            const playerStats = await query(`SELECT 
           ...
           AND p.userId IN (
            ...
            const eventPopularity = await query(`SELECT 
            e.name, e.type,
           ...
           ORDER BY timesPlayed DESC`, { parameters: [input.tournamentId] });
            // Get engagement metrics
            const engagement = await query(`SELECT 
           ...
           WHERE s._type = 'score_submission' ${dateFilter}`, { parameters: [input.tournamentId] });
            return {
        ...
        try {
            ...
            switch (input.action) {
                case 'remove': {
                    ...
                    const matchesPlayed = await query(`SELECT COUNT(*) as count FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
               WHERE _type = 'match' 
               AND (teamA = $1 OR teamB = $1)
               AND isComplete = true`, { parameters: [input.teamId] });
                ...
                case 'edit':
                    if (input.updates) {
                        if (input.updates.name)
                            team.name = input.updates.name;
                        if (input.updates.colorHex)
                            team.colorHex = input.updates.colorHex;
                        if (input.updates.flagCode)
                            team.flagCode = input.updates.flagCode;
                    }
                    break;
                case 'warn':
    ...
    getAdminLog: protectedProcedure
        ...
        try {
            ...
            const result = await query(`SELECT a.*, u.name as adminName
    ...
    exportTournamentData: protectedProcedure
        ...
        try {
            ...
            const [teams, matches, events, scores] = await Promise.all([
                query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
             WHERE _type = 'team' AND tournamentId = $1`, { parameters: [input.tournamentId] }),
                query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
             WHERE _type = 'match' AND tournamentId = $1`, { parameters: [input.tournamentId] }),
                query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
             WHERE _type = 'event' AND tournamentId = $1`, { parameters: [input.tournamentId] }),
                query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
             WHERE _type = 'score_entry' AND tournamentId = $1`, { parameters: [input.tournamentId] })
            ]);
            let playerData = null;
            if (input.includePlayerData) {
                // Get all player IDs from teams
                const allPlayerIds = (teams.rows || []).flatMap((team) => team.memberIds || []);
                const uniquePlayerIds = [...new Set(allPlayerIds)];
                const playerProfiles = await Promise.all(uniquePlayerIds.map(id => getDocument(`player_profile::${id}`)));
...
function convertToCSV(data) {
    ...
    const rows = teams.map((team) => Object.values(team).join(','));
