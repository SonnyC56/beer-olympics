...
async function updateTournamentState(match, winnerId) {
    const tournament = await getDocument(`tournament::${match.tournamentId}`);
    ...
    if (match.tournamentMatchId) {
        const teams = await query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
       WHERE _type = 'team' AND tournamentId = $1`, { parameters: [match.tournamentId] });
        // const winnerTeam = teams.rows.find((t: Team) => t.id === winnerId); // Not used currently
        const winnerIndex = teams.rows.findIndex((t) => t.id === winnerId) + 1;
...
async function updatePlayerStats(match, winnerId, scores) {
    const winnerTeam = await getDocument(`team::${winnerId}`);
    const loserTeamId = match.teamA === winnerId ? match.teamB : match.teamA;
    const loserTeam = loserTeamId ? await getDocument(`team::${loserTeamId}`) : null;
    if (!winnerTeam)
        return;
    // Update stats for all players
    const allPlayerIds = [...winnerTeam.memberIds, ...(loserTeam?.memberIds || [])];
    for (const playerId of allPlayerIds) {
        const profile = await getDocument(`player_profile::${playerId}`);
        if (!profile)
            continue;
        ...
        if (match.eventId) {
            ...
            if (event) {
                ...
                // Recalculate average
                const allGameScores = await query(`SELECT m.finalScore FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default m
           WHERE m._type = 'match' 
           AND m.eventId = $1
           ...
           AND (m.teamA IN (SELECT id FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default WHERE _type = 'team' AND $2 IN memberIds)
                OR m.teamB IN (SELECT id FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default WHERE _type = 'team' AND $2 IN memberIds))`, { parameters: [match.eventId, playerId] });
                let totalScore = 0;
                let count = 0;
                allGameScores.rows.forEach((row) => {
...
async function updateMegaTournamentScores(match, _winnerId, _scores) {
    const tournament = await getDocument(`tournament::${match.tournamentId}`);
    if (!tournament || !tournament.parentTournamentId) {
        return; // Not a sub-tournament
    }
    const megaTournament = await getDocument(`tournament::${tournament.parentTournamentId}`);
    ...
    for (const teamId of teams) {
        ...
        let megaScore = await getDocument(scoreId);
        if (!megaScore) {
            megaScore = {
...
export const enhancedMatchRouter = router({
    ...
    submitScore: protectedProcedure
        ...
        try {
            ...
            const userTeams = await query(`SELECT id FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'team' AND $1 IN memberIds`, { parameters: [ctx.user.id] });
            const userTeamIds = userTeams.rows.map((row) => row.id);
    ...
    getMatchDetails: publicProcedure
        ...
        try {
            ...
            const [teamA, teamB] = await Promise.all([
                match.teamA ? getDocument(`team::${match.teamA}`) : null,
                match.teamB ? getDocument(`team::${match.teamB}`) : null
            ]);
            // Get event details
            const event = match.eventId ? await getDocument(`event::${match.eventId}`) : null;
            // Get score submission if pending
            const pendingSubmission = await query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'score_submission' AND matchId = $1 AND status = 'PENDING'
           LIMIT 1`, { parameters: [input.matchId] });
            // Get media
            const media = match.mediaIds.length > 0 ? await Promise.all(match.mediaIds.map(id => getDocument(`media::${id}`))) : [];
            // Get head-to-head history
            let h2hHistory = [];
            if (teamA && teamB) {
                const h2hResult = await query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
             WHERE _type = 'match' 
             AND isComplete = true
             AND ((teamA = $1 AND teamB = $2) OR (teamA = $2 AND teamB = $1))
             AND id != $3
             ORDER BY endTime DESC
             LIMIT 5`, { parameters: [teamA.id, teamB.id, input.matchId] });
    ...
    getUpcomingMatches: publicProcedure
        ...
        try {
          ...
          AND m.isComplete = false`;
            const params = [input.tournamentId];
            if (input.teamId) {
    ...
    resolveDispute: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
        try {
            const dispute = await getDocument(`dispute::${input.disputeId}`);
            if (!dispute) {
                throw new TRPCError({
                    code: 'NOT_FOUND',
            ...
            const match = await getDocument(`match::${dispute.matchId}`);
            const tournament = await getDocument(`tournament::${match.tournamentId}`);
            ...
            const submission = await getDocument(`score_submission::${dispute.submissionId}`);
    ...
    getMatchTimeline: publicProcedure
        ...
        try {
            ...
            // Score submissions
            const submissions = await query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'score_submission' AND matchId = $1
           ORDER BY createdAt`, { parameters: [input.matchId] });
    ...
    getActiveMatches: publicProcedure
        ...
        try {
           ...
           ORDER BY m.startTime DESC`, { parameters: [input.tournamentId] });
...
function estimateMatchStartTime(match) {
