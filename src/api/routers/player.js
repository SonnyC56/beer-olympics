...
export const playerRouter = router({
    ...
    getOrCreateProfile: protectedProcedure
        .query(async ({ ctx }) => {
        try {
            let profile = await getDocument(`player_profile::${ctx.user.id}`);
            if (!profile) {
                // Create new profile
                profile = {
    ...
    updateProfile: protectedProcedure
        ...
        try {
            const profile = await getDocument(`player_profile::${ctx.user.id}`);
            if (!profile) {
                throw new TRPCError({
                    code: 'NOT_FOUND',
            ...
            if (input.favoriteGame !== undefined)
                profile.favoriteGame = input.favoriteGame;
            if (input.motto !== undefined)
                profile.motto = input.motto;
            if (input.socialLinks)
                profile.socialLinks = { ...profile.socialLinks, ...input.socialLinks };
    ...
    getPublicProfile: publicProcedure
        ...
        .query(async ({ input }) => {
        try {
            const profile = await getDocument(`player_profile::${input.userId}`);
            if (!profile) {
                throw new TRPCError({
                    code: 'NOT_FOUND',
                    message: 'Player profile not found',
    ...
    getDetailedStats: publicProcedure
        ...
        try {
            ...
            const teamsResult = await query(`SELECT id FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'team' AND $1 IN memberIds`, { parameters: [input.userId] });
            ...
            const matchesResult = await query(`SELECT m.*, e.name as eventName 
           ...
           ${dateFilter}
           ORDER BY m.endTime DESC`, { parameters: [teamIds] });
            // Calculate performance metrics
            const matches = matchesResult.rows;
            const recentPerformance = matches.slice(0, 10).map((match) => {
            ...
            Object.values(gameBreakdown).forEach(stats => {
                stats.averageScore = stats.gamesPlayed > 0 ?
                    matches
                        .filter((m) => m.eventName === Object.keys(gameBreakdown).find(k => gameBreakdown[k] === stats))
                        .reduce((sum, m) => {
    ...
    searchPlayers: publicProcedure
        ...
        try {
           ...
           LIMIT $3`, { parameters: [`%${input.query}%`, input.query, input.limit] });
    ...
    compareProfiles: publicProcedure
        ...
        try {
            const [profile1, profile2] = await Promise.all([
                getDocument(`player_profile::${input.userId1}`),
                getDocument(`player_profile::${input.userId2}`)
            ]);
            ...
            const teamsResult1 = await query(`SELECT id FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'team' AND $1 IN memberIds`, { parameters: [input.userId1] });
            const teamsResult2 = await query(`SELECT id FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'team' AND $1 IN memberIds`, { parameters: [input.userId2] });
            const teamIds1 = teamsResult1.rows.map((row) => row.id);
            const teamIds2 = teamsResult2.rows.map((row) => row.id);
           ...
           ORDER BY endTime DESC`, { parameters: [teamIds1, teamIds2] });
            let player1Wins = 0;
            let player2Wins = 0;
