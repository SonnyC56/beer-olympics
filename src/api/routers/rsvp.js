...
export const rsvpRouter = router({
    ...
    getStats: publicProcedure
        ...
        `;
            ...
            const skillLevels = skillResult.rows.reduce((acc, row) => {
        ...
        `;
            ...
            const shirtSizes = shirtResult.rows.reduce((acc, row) => {
    ...
    getCheckInStatus: publicProcedure
        ...
        `;
            ...
            const stats = {
                ...
                noShows: attendees.filter((a) => a.status === 'no_show').length,
                capacity: 64, // Default capacity
                teamsFormed: Math.floor(attendees.filter((a) => a.status === 'checked_in' && a.teamId).length / 2),
                lateArrivals: attendees.filter((a) => a.isLateArrival).length,
            };
            return {
                success: true,
                attendees,
...
async function findOrCreateTeam(tournamentSlug, rsvp) {
    try {
