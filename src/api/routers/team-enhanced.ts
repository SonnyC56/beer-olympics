...
import type { Team, Tournament, PlayerProfile, User, TeamStats } from '../../types';
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const tournament = await getDocument(`tournament::${input.tournamentSlug}`) as Tournament;
        ...
        const existingTeams = await query(
          ...
          { parameters: [input.tournamentSlug, input.name] }
        );
      ...
      try {
        ...
        const teamsResult = await query(
          `SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'team' AND inviteCode = $1`,
          { parameters: [input.inviteCode] }
        );
        ...
        const team = teamsResult.rows[0] as Team;
        
        // Check if tournament is still open
        const tournament = await getDocument(`tournament::${team.tournamentId}`) as Tournament;
        if (!tournament?.isOpen) {
          throw new TRPCError({
            code: 'FORBIDDEN',
            message: 'Tournament registration is closed',
    ...
    .query(async ({ input }) => {
      try {
        ...
        const playerProfiles = await Promise.all(
          team.memberIds.map(async (userId) => {
            const profile = await getDocument(`player_profile::${userId}`) as PlayerProfile;
            const user = await getDocument(`user::${userId}`) as User;
            return {
              ...profile,
              user: {
                id: user?.id,
        ...
        if (input.updates.name && input.updates.name !== team.name) {
          const existingTeams = await query(
            `SELECT name FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
             ...
             AND id != $3`,
            { parameters: [team.tournamentId, input.updates.name, input.teamId] }
          );
  ...
  getTeamStats: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        let longestStreak = 0;
        let tempStreak = 0;
        const gameTypeStats: { [key: string]: any } = {};
        const opponentStats: { [key: string]: any } = {};
        ...
        let nemesisTeam = '';
        let maxLosses = 0;
        Object.entries(opponentStats).forEach(([teamId, stats]: [string, any]) => {
        ...
        const updatedStats: TeamStats = {
          ...
          favoriteGame: Object.entries(gameTypeStats).sort((a: any, b: any) => b[1].wins - a[1].wins)[0]?.[0],
          nemesisTeam
        };
        ...
        return {
          ...
          recentForm: matches.slice(0, 5).map((m: any) => {
