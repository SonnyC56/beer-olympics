...
export const enhancedTournamentRouter = router({
    ...
    getStats: publicProcedure
        ...
        try {
           ...
           WHERE _type = 'match' AND tournamentId = $1`, { parameters: [input.tournamentId] });
            ...
            // Get teams count
            const teamsResult = await query(`SELECT COUNT(*) as count FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'team' AND tournamentId = $1`, { parameters: [input.tournamentId] });
            // Get unique games played
            const gamesResult = await query(`SELECT COUNT(DISTINCT eventId) as count FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'match' AND tournamentId = $1 AND isComplete = true`, { parameters: [input.tournamentId] });
    ...
    create: protectedProcedure
        ...
        try {
            ...
            if (input.parentTournamentId) {
                const parent = await getDocument(`tournament::${input.parentTournamentId}`);
    ...
    startTournament: protectedProcedure
        ...
        try {
           ...
           ORDER BY createdAt`, { parameters: [input.slug] });
        ...
        try {
            ...
            const [teamsResult, matchesResult] = await Promise.all([
             ...
             WHERE _type = 'team' AND tournamentId = $1`, { parameters: [input.slug] }),
                query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
             WHERE _type = 'match' AND tournamentId = $1`, { parameters: [input.slug] })
            ]);
            const teams = teamsResult.rows;
            const matches = matchesResult.rows;
            // Calculate standings
    ...
    getStatsBySlug: publicProcedure
        ...
        try {
            ...
            const matches = matchesResult.rows;
            const completedMatches = matches.filter(m => m.isComplete);
            const totalPoints = completedMatches.reduce((sum, m) => sum + (m.finalScore?.a || 0) + (m.finalScore?.b || 0), 0);
            const stats = {
                tournamentId: input.slug,
    ...
    listTeams: publicProcedure
        ...
        .query(async ({ input }) => {
        try {
            const result = await query(`SELECT * FROM \`${process.env.COUCHBASE_BUCKET || 'beer_olympics'}\`._default._default
           WHERE _type = 'team' AND tournamentId = $1
           ORDER BY createdAt`, { parameters: [input.tournamentId] });
            return result.rows;
        }
        catch (error) {
            throw new TRPCError({
                ...
                settings: {
                    megaTournamentScoring: {
                        ...
                        placementPoints: input.subTournaments[0]?.pointsForPlacement
                            ? Object.fromEntries(Object.entries(input.subTournaments[0].pointsForPlacement).map(([k, v]) => [Number(k), v]))
    ...
    getMegaTournamentLeaderboard: publicProcedure
        ...
        try {
           ...
           ORDER BY totalPoints DESC`, { parameters: [input.slug] });
            return result.rows;
        }
        catch (error) {
            if (error instanceof TRPCError)
    ...
    completeBonusChallenge: protectedProcedure
        ...
        try {
            const tournament = await getDocument(`tournament::${input.megaTournamentSlug}`);
            if (!tournament || !tournament.isMegaTournament) {
                throw new TRPCError({
                    code: 'NOT_FOUND',
                    message: 'Mega tournament not found',
            ...
            let score = await getDocument(scoreId);
            if (!score) {
                score = {
            ...
            const completions = score.bonusCompletions.filter((c) => c.challengeId === input.challengeId);
    ...
    getLeaderboard: publicProcedure
        ...
        try {
           ...
           WHERE _type = 'team' AND tournamentId = $1`, { parameters: [input.tournamentId] });
            // Get score totals
            const scoresResult = await query(`SELECT teamId, SUM(points) as totalPoints, COUNT(*) as matchesPlayed,
           ...
           GROUP BY teamId`, { parameters: [input.tournamentId] });
            // Create score map
            ...
            const leaderboard = teamsResult.rows.map((team, index) => {
...
// Helper functions
function getRoundName(roundNumber, format) {
...
function calculateStandings(teams, matches, tournament) {
