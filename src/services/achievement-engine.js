...
export class AchievementEngine {
    constructor(db) {
        ...
        Object.defineProperty(this, "progressCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    async checkAchievements(userId, context) {
        ...
        for (const [id, definition] of Object.entries(ACHIEVEMENTS)) {
            ...
            if (definition.prerequisiteIds?.length) {
                const hasAllPrereqs = definition.prerequisiteIds.every(prereqId => currentAchievements.has(prereqId));
                if (!hasAllPrereqs)
                    continue;
            }
            // Check if criteria is met
            const isMet = await this.checkCriteria(userId, definition.criteria, context);
            if (isMet) {
     ...
     */
    async checkCriteria(userId, criteria, context) {
     ...
     */
    async getCriteriaValue(userId, criteria, context) {
     ...
     */
    async unlockAchievement(userId, definition) {
        const achievement = {
            id: definition.id,
            type: definition.category === 'performance' ? 'milestone' : definition.category,
    ...
    async updateProgress(userId, updates) {
        for (const update of updates) {
            ...
            progress.currentValue = Math.min(progress.currentValue + update.increment, progress.targetValue);
    ...
    async isFirstTimeGlobally(achievementId) {
        // Check if anyone else has this achievement
        return false;
    }
    async getOrCreateProgress(userId, achievementId) {
