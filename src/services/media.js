...
class MediaService {
    ...
    async uploadFile(file, options) {
        ...
        try {
            const result = await cloudinary.uploader.upload(typeof file === 'string' ? file : `data:${type === 'video' ? 'video/mp4' : 'image/jpeg'};base64,${file.toString('base64')}`, uploadOptions);
     ...
     */
    async getTournamentMedia(tournamentId) {
        try {
            const result = await cloudinary.search
     ...
     */
    async generateHighlightReel(tournamentId, videoIds) {
        try {
     ...
     */
    async detectHighlights(mediaIds) {
        const highlights = {
            fastestChug: [],
            biggestUpset: [],
            funnyMoments: [],
        ...
        try {
            ...
            for (const mediaId of mediaIds) {
                ...
                if (tags.some((tag) => ['drinking', 'beer', 'fast', 'speed', 'chug', 'competition'].includes(tag.toLowerCase()))) {
                    highlights.fastestChug.push(mediaId);
                }
                // Check for upset indicators (celebration, surprise)
                if (tags.some((tag) => ['celebration', 'victory', 'surprise', 'upset', 'winner'].includes(tag.toLowerCase()))) {
                    highlights.biggestUpset.push(mediaId);
                }
                // Check for funny moments
                if (tags.some((tag) => ['funny', 'laugh', 'fail', 'blooper', 'comedy'].includes(tag.toLowerCase()))) {
