...
import { 
  ...
  ConnectionMetrics,
  BaseRealtimeService 
} from './realtime-enhanced';

// Scalable WebSocket service with connection pooling and advanced features
export class ScalableWebSocketService extends BaseRealtimeService implements EnhancedRealtimeService {
  ...
  private presenceData = new Map<string, Map<string, any>>(); // channel -> members
  ...
  subscribe(channelName: string): RealtimeChannel | null {
    if (!this.connectionPool) return null;
  ...
  subscribePresence(channelName: string): PresenceChannel | null {
    if (!channelName.startsWith('presence-')) return null;
  ...
  private sendToChannel(channel: string, event: string, data: any): void {
    // Queue message
  ...
  private handlePoolMessage(message: any): void {
  ...
  private updatePresenceData(channel: string, data: any): void {
  ...
  private async flushMessageQueue(): Promise<void> {
    if (!this.connectionPool || this.messageQueue.getStats().total === 0) return;
  ...
  private getChannelAuth(channel: string): any {
  ...
  private getMessagePriority(event: string): 'low' | 'normal' | 'high' | 'critical' {
    // Prioritize messages based on event type
    if (event.includes('score') || event.includes('winner')) return 'critical';
    if (event.includes('update') || event.includes('status')) return 'high';
    if (event.includes('join') || event.includes('leave')) return 'normal';
    return 'low';
  }
  
  private shouldPersistMessage(event: string): boolean {
