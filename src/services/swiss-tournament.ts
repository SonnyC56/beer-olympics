import type { Tournament, Match, Team, Standing } from '../types/tournament';
...
export class SwissTournamentService {
  private players: Map<string, SwissPlayer> = new Map();
  private pairings: SwissPairing[] = [];
  private currentRound: number = 1;
  private maxRounds: number;

  constructor(teams: Team[], maxRounds: number = 0) {
    // Initialize players
    teams.forEach(team => {
    ...
    if (maxRounds === 0) {
      ...
      if (playerCount <= 8) this.maxRounds = 3;
      else if (playerCount <= 16) this.maxRounds = 4;
      else if (playerCount <= 32) this.maxRounds = 5;
      else if (playerCount <= 64) this.maxRounds = 6;
      else this.maxRounds = 7;
    } else {
  ...
  generatePairings(round: number): SwissPairing[] {
    ...
    const sortedPlayers = Array.from(this.players.entries())
      .sort(([, a], [, b]) => {
        ...
        if (buchholzB !== buchholzA) return buchholzB - buchholzA;
    ...
    for (const playerId of sortedPlayers) {
      if (!availablePlayers.has(playerId)) continue;
      ...
      for (const opponentId of sortedPlayers) {
        ...
        if (!availablePlayers.has(opponentId)) continue;
        if (player.opponents.includes(opponentId)) continue;
   ...
   */
  updateMatchResult(player1Id: string, player2Id: string | null, winner: string | null) {
   ...
   */
  private calculateBuchholz(playerId: string): number {
    const player = this.players.get(playerId)!;
    let buchholz = 0;
   ...
   */
  private calculateSonnebornBerger(playerId: string): number {
