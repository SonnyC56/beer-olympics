...
export class TournamentEngine {
    constructor(config) {
        ...
        Object.defineProperty(this, "playerMap", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "schedulingEngine", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "swissEngine", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
    ...
    getPlayerSeedByName(playerName) {
    ...
    scoreMatch(matchId, scores) {
        ...
        if (this.config.format === 'swiss' && this.swissEngine) {
            ...
            const match = matches.find(m => m.id.s === matchId.s && m.id.r === matchId.r && m.id.m === matchId.m);
    ...
    getTotalRounds() {
        ...
        if (!this.tournament)
            return 1;
        return Math.max(...this.tournament.matches.map((m) => m.id.r));
    }
    /**
    ...
    generateSwissNextRound() {
        ...
        const matches = this.swissEngine.pairingsToMatches(pairings, this.stations.map(s => s.id), { id: 'default' } // Game info
        );
     ...
     */
    resolveTiebreaker(players, method = 'head2head') {
        if (!this.tournament) {
            throw new Error('Tournament not initialized');
        }
        try {
    ...
    static importState(state) {
        ...
        if (state.tournamentState && engine.tournament) {
            ...
            state.tournamentState.matches.forEach((match) => {
                if (match.m) {
     ...
     */
    async generateSchedule(schedulingConfig, constraints = []) {
        if (!this.schedulingEngine) {
        ...
        const result = await this.schedulingEngine.generateSchedule(tournamentMatches, // Type assertion for simplicity
        constraints);
        if (result.success) {
     ...
     * Handle match delay
     */
    async handleMatchDelay(slotId, delayMinutes) {
        if (!this.schedulingEngine) {
     ...
     */
    recommendStationsForMatch(matchId) {
