...
export class CacheWarmer {
  ...
  async warmCache(options: CacheWarmingOptions = {}): Promise<void> {
    ...
    try {
      const {
        tournamentIds = [],
        teamIds = [],
        warmLeaderboards = true,
        warmTournaments = true,
        warmTeams = true,
        warmStats = true,
      } = options;
      ...
      if (warmLeaderboards) {
        for (const tournamentId of activeTournamentIds) {
          warmingTasks.push(
            this.warmLeaderboard(tournamentId).catch(err => 
              console.error(`Failed to warm leaderboard for ${tournamentId}:`, err)
            )
          );
        }
      }
      ...
      if (warmTournaments) {
        for (const tournamentId of activeTournamentIds) {
          warmingTasks.push(
            this.warmTournamentData(tournamentId).catch(err =>
              console.error(`Failed to warm tournament data for ${tournamentId}:`, err)
            )
          );
        }
      }
      ...
      if (warmTeams && teamIds.length > 0) {
        for (const teamId of teamIds) {
          warmingTasks.push(
            this.warmTeamData(teamId).catch(err =>
              console.error(`Failed to warm team data for ${teamId}:`, err)
            )
          );
        }
      }
      ...
      if (warmStats) {
        warmingTasks.push(
          this.warmGlobalStats().catch(err =>
            console.error('Failed to warm global stats:', err)
          )
        );
      }
   ...
   */
  startPeriodicWarming(intervalMs = 60000, options: CacheWarmingOptions = {}): void {
   ...
   */
  private async getActiveTournamentIds(): Promise<string[]> {
    try {
      ...
      return result.rows?.map((row: any) => row.tournament_id) || [];
    } catch (error) {
   ...
   */
  private async warmLeaderboard(tournamentId: string): Promise<void> {
   ...
   * Warm tournament data
   */
  private async warmTournamentData(tournamentId: string): Promise<void> {
   ...
   * Warm team data
   */
  private async warmTeamData(teamId: string): Promise<void> {
   ...
   */
  private async warmGlobalStats(): Promise<void> {
