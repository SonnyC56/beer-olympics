...
// Message queue for offline scenarios and retry logic
export interface QueuedMessage {
  ...
  persistent?: boolean; // Should survive page reload
}
...
export class MessageQueue extends EventEmitter {
  ...
  private priorityIndex: Map<string, QueuedMessage[]> = new Map([
    ['critical', []],
    ['high', []],
    ['normal', []],
    ['low', []],
  ...
  // Process queue
  async processQueue(sendFunction: (messages: QueuedMessage[]) => Promise<boolean>): Promise<void> {
  ...
  private removeMessage(message: QueuedMessage): void {
  ...
  // Remove oldest low priority message
  private removeOldestLowPriority(): boolean {
    const priorities: Array<QueuedMessage['priority']> = ['low', 'normal', 'high', 'critical'];
    
    for (const priority of priorities) {
      const messages = this.priorityIndex.get(priority)!;
      if (messages.length > 0) {
        const oldest = messages.reduce((prev, curr) => 
...
export class MessageDeduplicator {
  ...
  private generateKey(channel: string, event: string, data: any): string {
